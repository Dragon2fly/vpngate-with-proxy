...
class Server():
    ...
    def __str__(self):
        ...
        txt = [self.country_short, str(self.ping), '%.2f' % speed, uptime, self.logPolicy, str(self.score)]
...
labels = ['Index', 'Country', 'Ping', 'Speed', 'Up time', 'Log Policy', 'Score']
spaces = [6, 7, 6, 10, 10, 10, 10]
...
while True:
    ...
    try:
        ...
        if user_input in ['Q', 'q', 'quit', 'exit']:
            sys.exit()
        elif user_input in ['r', 'R', 'refresh']:
            ranked, vpn_list = refresh_data()
